...
df = pd.read_csv('g4_scores_final.bed', header=None, index_col=None, sep='\t')
df['score'] = df[2]/df[2].max()

with open('g4_scores_final_norm.bed', 'w') as f:
    for pqs, strand, score in zip(df[0], df[1], df['score']):
        f.write(f'{pqs}\t{strand}\t{score}\n')

df = pd.read_csv('epi_final.bed', header=None, index_col=None, sep='\t')
df['score'] = df[5]/df[5].max()

with open('epi_final_norm.bed', 'w') as f:
